          ...
          <form onSubmit={handleSubmit} className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 md:p-8 rounded-2xl shadow-lg text-gray-900 dark:text-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label className="block">
                ...
                <input id="name" name="name" required value={form.name} onChange={handleChange} className="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </label>

              <label className="block">
                <div className="text-sm mb-1 font-medium text-gray-700 dark:text-gray-300">{t('contactFormEmailLabel')}</div>
                <input id="email" name="email" type="email" required value={form.email} onChange={handleChange} className={`w-full border ${form.email && !emailValid ? 'border-red-300' : 'border-gray-200'} dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500`} />
            ...
            <label className="block mt-4">
              ...
              <select id="type" name="type" value={form.type} onChange={handleChange} className="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <label className="block mt-4">
              ...
              <textarea id="message" name="message" required value={form.message} onChange={handleChange} rows={6} className="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </label>
            ...
            <div className="mt-6">
              <button
                ...
                className={`inline-flex items-center justify-center w-full md:w-auto px-6 py-2 rounded-lg font-semibold transition ${status === 'sending' ? 'bg-indigo-400 cursor-wait' : formValid ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-300 text-gray-600 cursor-not-allowed'} text-white`}
              >
                {status === 'sending' ? (
